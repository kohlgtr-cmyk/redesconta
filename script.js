...
const firebaseConfig = {
    apiKey: "AIzaSyAYY84LKYVlcrjaVoxMnC4uxWmgeYD2Vj0",
    authDomain: "redesconta-4efcf.firebaseapp.com",
    projectId:  "redesconta-4efcf",
    databaseURL: "https://redesconta-4efcf-default-rtdb.firebaseio.com",  // IMPORTANTE!
    storageBucket: "redesconta-4efcf.firebasestorage.app",
    messagingSenderId:  "485360003323",
    appId: "1:485360003323:web:8f42b4f6002ed34982a85e"
};

// Initialize Firebase
firebase.initializeApp(firebaseConfig);
const auth = firebase.auth();
const database = firebase.database();  // MUDOU: database ao invés de firestore
...
async function loadRetiros() {
    ...
    try {
        const snapshot = await database.ref('retiros').orderByChild('timestamp').once('value');
        ...
        const retiros = Object.keys(data).map(key => ({
            id: key,
            ...data[key]
            ...
            ${retiro.coords ? `
                <div class="map-container">
                    <iframe 
                        ...
                        frameborder="0" 
                        style="border:0" 
                        src="https://maps.google.com/maps?q=${retiro.coords}&z=15&output=embed">
                    </iframe>
                </div>` : ''}
        </div>
    </div>
...
async function loadGaleria() {
    ...
    try {
        ...
        Object.values(data).forEach(photo => {
            if (!photosByEdicao[photo.edicao]) {
                photosByEdicao[photo.edicao] = [];
            }
            photosByEdicao[photo.edicao].push(photo.url);
        });
...
async function saveRetiro(e) {
    ...
    try {
        const id = document.getElementById('retiroId').value || database.ref('retiros').push().key;
        ...
        if (imageFile) {
            ...
            const storageRef = storage.ref(`retiros/${id}_${imageFile.name}`);
        ...
        await database.ref('retiros/' + id).set(retiroData);
        
        await loadRetiros();
        await loadAdminRetiros();
        resetForm();
        showAlert('adminAlert', 'Retiro salvo com sucesso!', 'success');
    } catch (error) {
...
async function deleteRetiro(id) {
    ...
    try {
        await database.ref('retiros/' + id).remove();
        await loadRetiros();
        await loadAdminRetiros();
...
async function uploadPhotos() {
    ...
    try {
        ...
        for (const file of selectedPhotos) {
            const photoId = database.ref('photos').push().key;
            const storageRef = storage.ref(`galeria/${edicao}/${photoId}_${file.name}`);
            ...
            await database.ref('photos/' + photoId).set({
                edicao: edicao,
                url: url,
                timestamp: Date.now()
            });
